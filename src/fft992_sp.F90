...
!      
      ...
      INTEGER(KIND=4) :: JUMP
      INTEGER(KIND=4) :: J,JJ,JUMPA
      INTEGER(KIND=4) :: LOT
      INTEGER(KIND=4) :: K,LA,NFAX
      INTEGER(KIND=4) :: ISIGN
      INTEGER(KIND=4) :: I,IA,IBASE,IERR,IFAC,IGO,II,INCA,IX

      REAL(KIND=4) :: A(*)
      REAL(KIND=4) :: TRIGS(N)
      INTEGER(KIND=4) :: IFAX(10)
!     Dynamically allocated work array:
      REAL(KIND=4) :: WORK(N*LOT+1)
      LOGICAL :: LIPL
!
      NFAX=IFAX(1)
      IF (ISIGN.EQ.+1) THEN
...
!
      ...
      REAL(KIND=4) :: D(*)
      REAL(KIND=4) :: TRIGS(N)
      REAL(KIND=4) :: A10,A11,A20,A21
      REAL(KIND=4) :: B10,B11,B20,B21
      REAL(KIND=4) :: C1,C2,C3,C4,C5
      REAL(KIND=4) :: S1,S2,S3,S4,S5
      REAL(KIND=4) :: SIN36,SIN45,SIN60,SIN72
      REAL(KIND=4) :: SSIN36,SSIN45,SSIN60,SSIN72
      REAL(KIND=4) :: QRT5,QQRT5
      REAL(KIND=4) :: T1,T2,T3,T4,T5,T6,T7
      INTEGER(KIND=4) :: IERR
      INTEGER(KIND=4) :: INC1
      INTEGER(KIND=4) :: INC2
      INTEGER(KIND=4) :: INC3
      ...
      INTEGER(KIND=4) :: IFAC
      INTEGER(KIND=4) :: LA
      INTEGER(KIND=4) :: INC21,IINK,IJK,ILOT,ILA
      INTEGER(KIND=4) :: I,IA,IB,IBAD,IBASE,IC,ID,IE,IF
      INTEGER(KIND=4) :: J,JA,JB,JBASE,JC,JD,JE,JF,JG,JH,JINK,JUMP
      INTEGER(KIND=4) :: K,KB,KC,KD,KE,KF,KSTOP
      INTEGER(KIND=4) :: L,M
      LOGICAL :: LIPL
!
      DATA SIN36/0.587785252292473_4/,SIN72/0.951056516295154_4/, &
     &    QRT5/0.559016994374947_4/,SIN60/0.866025403784437_4/
!
      M=N/IFAC
      IINK=LA*INC1
      JINK=LA*INC2
...
!NEC$ ivdep
        DO 296 IJK=1,ILOT
        C(JA+J)=2.0_4*(A(IA+I)+A(IB+I))
        C(JB+J)=2.0_4*(A(IA+I)-A(IB+I))
        I=I+INC3
        J=J+INC4
  296   CONTINUE
        IBASE=IBASE+INC1
...
!NEC$ ivdep
      DO 310 IJK=1,ILOT
      C(JA+J)=A(IA+I)+A(IB+I)
      C(JB+J)=(A(IA+I)-0.5_4*A(IB+I))-(SIN60*(B(IB+I)))
      C(JC+J)=(A(IA+I)-0.5_4*A(IB+I))+(SIN60*(B(IB+I)))
      I=I+INC3
      J=J+INC4
  310 CONTINUE
      IBASE=IBASE+INC1
...
!NEC$ ivdep
      ...
      C(JB+J)=                                                          &
     &    C1*((A(IA+I)-0.5_4*(A(IB+I)+A(IC+I)))-                     &
     &   (SIN60*(B(IB+I)+B(IC+I))))                                     &
     &   -S1*((B(IA+I)-0.5_4*(B(IB+I)-B(IC+I)))+                     &
     &   (SIN60*(A(IB+I)-A(IC+I))))
      D(JB+J)=                                                          &
     &    S1*((A(IA+I)-0.5_4*(A(IB+I)+A(IC+I)))-                     &
     &   (SIN60*(B(IB+I)+B(IC+I))))                                     &
     &   +C1*((B(IA+I)-0.5_4*(B(IB+I)-B(IC+I)))+                     &
     &   (SIN60*(A(IB+I)-A(IC+I))))
      C(JC+J)=                                                          &
     &    C2*((A(IA+I)-0.5_4*(A(IB+I)+A(IC+I)))+                     &
     &   (SIN60*(B(IB+I)+B(IC+I))))                                     &
     &   -S2*((B(IA+I)-0.5_4*(B(IB+I)-B(IC+I)))-                     &
     &   (SIN60*(A(IB+I)-A(IC+I))))
      D(JC+J)=                                                          &
     &    S2*((A(IA+I)-0.5_4*(A(IB+I)+A(IC+I)))+                     &
     &   (SIN60*(B(IB+I)+B(IC+I))))                                     &
     &   +C2*((B(IA+I)-0.5_4*(B(IB+I)-B(IC+I)))-                     &
     &   (SIN60*(A(IB+I)-A(IC+I))))
      I=I+INC3
      J=J+INC4
  330 CONTINUE
...
!NEC$ ivdep
      DO 370 IJK=1,ILOT
      C(JA+J)=A(IA+I)+A(IB+I)
      C(JB+J)=(0.5_4*A(IA+I)-A(IB+I))-(SIN60*B(IA+I))
      C(JC+J)=-(0.5_4*A(IA+I)-A(IB+I))-(SIN60*B(IA+I))
      I=I+INC3
      J=J+INC4
  370 CONTINUE
      IBASE=IBASE+INC1
...
!NEC$ ivdep
        DO 392 IJK=1,ILOT
        T1=(2.0_4*A(IA+I)-A(IB+I))-(SSIN60*B(IB+I))
        T2=(2.0_4*A(IA+I)-A(IB+I))+(SSIN60*B(IB+I))
        A(IA+I)=2.0_4*(A(IA+I)+A(IB+I))
        A(IB+I)=T1
        B(IB+I)=T2
        I=I+INC3
...
!NEC$ ivdep
        DO 396 IJK=1,ILOT
        C(JA+J)=2.0_4*(A(IA+I)+A(IB+I))
        C(JB+J)=(2.0_4*A(IA+I)-A(IB+I))-(SSIN60*B(IB+I))
        C(JC+J)=(2.0_4*A(IA+I)-A(IB+I))+(SSIN60*B(IB+I))
        I=I+INC3
        J=J+INC4
  396   CONTINUE
        IBASE=IBASE+INC1
...
!NEC$ ivdep
        DO 492 IJK=1,ILOT
        T1=2.0_4*((A(IA+I)-A(IC+I))-B(IB+I))
        T2=2.0_4*((A(IA+I)+A(IC+I))-A(IB+I))
        T3=2.0_4*((A(IA+I)-A(IC+I))+B(IB+I))
        A(IA+I)=2.0_4*((A(IA+I)+A(IC+I))+A(IB+I))
        A(IB+I)=T1
        B(IB+I)=T2
        A(IC+I)=T3
        I=I+INC3
...
!NEC$ ivdep
        DO 496 IJK=1,ILOT
        C(JA+J)=2.0_4*((A(IA+I)+A(IC+I))+A(IB+I))
        C(JB+J)=2.0_4*((A(IA+I)-A(IC+I))-B(IB+I))
        C(JC+J)=2.0_4*((A(IA+I)+A(IC+I))-A(IB+I))
        C(JD+J)=2.0_4*((A(IA+I)-A(IC+I))+B(IB+I))
        I=I+INC3
        J=J+INC4
  496   CONTINUE
        IBASE=IBASE+INC1
...
!NEC$ ivdep
      DO 510 IJK=1,ILOT
      C(JA+J)=A(IA+I)+(A(IB+I)+A(IC+I))
      C(JB+J)=((A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))+                   &
     &      QRT5*(A(IB+I)-A(IC+I)))-(SIN72*B(IB+I)+SIN36*B(IC+I))
      C(JC+J)=((A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))-                   &
     &      QRT5*(A(IB+I)-A(IC+I)))-(SIN36*B(IB+I)-SIN72*B(IC+I))
      C(JD+J)=((A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))-                   &
     &      QRT5*(A(IB+I)-A(IC+I)))+(SIN36*B(IB+I)-SIN72*B(IC+I))
      C(JE+J)=((A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))+                   &
     &      QRT5*(A(IB+I)-A(IC+I)))+(SIN72*B(IB+I)+SIN36*B(IC+I))
      I=I+INC3
      J=J+INC4
  510 CONTINUE
...
!NEC$ ivdep
      DO 530 IJK=1,ILOT
!
      A10=(A(IA+I)-0.25_4*((A(IB+I)+A(IE+I))+(A(IC+I)+A(ID+I))))     &
     &    +QRT5*((A(IB+I)+A(IE+I))-(A(IC+I)+A(ID+I)))
      A20=(A(IA+I)-0.25_4*((A(IB+I)+A(IE+I))+(A(IC+I)+A(ID+I))))     &
     &    -QRT5*((A(IB+I)+A(IE+I))-(A(IC+I)+A(ID+I)))
      B10=(B(IA+I)-0.25_4*((B(IB+I)-B(IE+I))+(B(IC+I)-B(ID+I))))     &
     &    +QRT5*((B(IB+I)-B(IE+I))-(B(IC+I)-B(ID+I)))
      B20=(B(IA+I)-0.25_4*((B(IB+I)-B(IE+I))+(B(IC+I)-B(ID+I))))     &
     &    -QRT5*((B(IB+I)-B(IE+I))-(B(IC+I)-B(ID+I)))
...
!NEC$ ivdep
        DO 592 IJK=1,ILOT
        T1=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))              &
     &    +QQRT5*(A(IB+I)-A(IC+I)))-(SSIN72*B(IB+I)+SSIN36*B(IC+I))
        T2=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))              &
     &    -QQRT5*(A(IB+I)-A(IC+I)))-(SSIN36*B(IB+I)-SSIN72*B(IC+I))
        T3=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))              &
     &    -QQRT5*(A(IB+I)-A(IC+I)))+(SSIN36*B(IB+I)-SSIN72*B(IC+I))
        T4=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))              &
     &    +QQRT5*(A(IB+I)-A(IC+I)))+(SSIN72*B(IB+I)+SSIN36*B(IC+I))
        A(IA+I)=2.0_4*(A(IA+I)+(A(IB+I)+A(IC+I)))
        A(IB+I)=T1
        B(IB+I)=T2
        A(IC+I)=T3
        B(IC+I)=T4
...
!NEC$ ivdep
        DO 596 IJK=1,ILOT
        C(JA+J)=2.0_4*(A(IA+I)+(A(IB+I)+A(IC+I)))
        C(JB+J)=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))         &
     &    +QQRT5*(A(IB+I)-A(IC+I)))-(SSIN72*B(IB+I)+SSIN36*B(IC+I))
        C(JC+J)=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))         &
     &    -QQRT5*(A(IB+I)-A(IC+I)))-(SSIN36*B(IB+I)-SSIN72*B(IC+I))
        C(JD+J)=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))         &
     &    -QQRT5*(A(IB+I)-A(IC+I)))+(SSIN36*B(IB+I)-SSIN72*B(IC+I))
        C(JE+J)=(2.0_4*(A(IA+I)-0.25_4*(A(IB+I)+A(IC+I)))         &
     &    +QQRT5*(A(IB+I)-A(IC+I)))+(SSIN72*B(IB+I)+SSIN36*B(IC+I))
        I=I+INC3
        J=J+INC4
  596   CONTINUE
...
!NEC$ ivdep
      ...
      C(JB+J)=((A(IA+I)-A(ID+I))+0.5_4*(A(IB+I)-A(IC+I)))            &
     &    -(SIN60*(B(IB+I)+B(IC+I)))
      C(JF+J)=((A(IA+I)-A(ID+I))+0.5_4*(A(IB+I)-A(IC+I)))            &
     &    +(SIN60*(B(IB+I)+B(IC+I)))
      C(JC+J)=((A(IA+I)+A(ID+I))-0.5_4*(A(IB+I)+A(IC+I)))            &
     &    -(SIN60*(B(IB+I)-B(IC+I)))
      C(JE+J)=((A(IA+I)+A(ID+I))-0.5_4*(A(IB+I)+A(IC+I)))            &
     &    +(SIN60*(B(IB+I)-B(IC+I)))
      I=I+INC3
      J=J+INC4
  610 CONTINUE
...
!NEC$ ivdep
     ...
     &        (0.5_4*(B(IA+I)+B(IC+I))+B(IB+I))
      C(JF+J)=-(SIN60*(A(IA+I)-A(IC+I)))-                               &
     &        (0.5_4*(B(IA+I)+B(IC+I))+B(IB+I))
      C(JC+J)=SIN60*(B(IC+I)-B(IA+I))+                                  &
     &        (0.5_4*(A(IA+I)+A(IC+I))-A(IB+I))
...
!NEC$ ivdep
        DO 692 IJK=1,ILOT
        T1=(2.0_4*(A(IA+I)-A(ID+I))+(A(IB+I)-A(IC+I)))               &
     &    -(SSIN60*(B(IB+I)+B(IC+I)))
        T5=(2.0_4*(A(IA+I)-A(ID+I))+(A(IB+I)-A(IC+I)))               &
     &    +(SSIN60*(B(IB+I)+B(IC+I)))
        T2=(2.0_4*(A(IA+I)+A(ID+I))-(A(IB+I)+A(IC+I)))               &
     &    -(SSIN60*(B(IB+I)-B(IC+I)))
        T4=(2.0_4*(A(IA+I)+A(ID+I))-(A(IB+I)+A(IC+I)))               &
     &    +(SSIN60*(B(IB+I)-B(IC+I)))
        T3=(2.0_4*(A(IA+I)-A(ID+I)))-(2.0_4*(A(IB+I)-A(IC+I)))
        A(IA+I)=(2.0_4*(A(IA+I)+A(ID+I)))+                           &
     &          (2.0_4*(A(IB+I)+A(IC+I)))
        A(IB+I)=T1
        B(IB+I)=T2
        A(IC+I)=T3
...
!NEC$ ivdep
        DO 696 IJK=1,ILOT
        C(JA+J)=(2.0_4*(A(IA+I)+A(ID+I)))+                           &
     &          (2.0_4*(A(IB+I)+A(IC+I)))
        C(JD+J)=(2.0_4*(A(IA+I)-A(ID+I)))-                           &
     &          (2.0_4*(A(IB+I)-A(IC+I)))
        C(JB+J)=(2.0_4*(A(IA+I)-A(ID+I))+(A(IB+I)-A(IC+I)))          &
     &    -(SSIN60*(B(IB+I)+B(IC+I)))
        C(JF+J)=(2.0_4*(A(IA+I)-A(ID+I))+(A(IB+I)-A(IC+I)))          &
     &    +(SSIN60*(B(IB+I)+B(IC+I)))
        C(JC+J)=(2.0_4*(A(IA+I)+A(ID+I))-(A(IB+I)+A(IC+I)))          &
     &    -(SSIN60*(B(IB+I)-B(IC+I)))
        C(JE+J)=(2.0_4*(A(IA+I)+A(ID+I))-(A(IB+I)+A(IC+I)))          &
     &    +(SSIN60*(B(IB+I)-B(IC+I)))
        I=I+INC3
        J=J+INC4
  696   CONTINUE
...
!NEC$ ivdep
        DO 810 IJK=1,ILOT
        T2=2.0_4*(((A(IA+I)+A(IE+I))-A(IC+I))-(B(IB+I)-B(ID+I)))
        T6=2.0_4*(((A(IA+I)+A(IE+I))-A(IC+I))+(B(IB+I)-B(ID+I)))
        T1=2.0_4*((A(IA+I)-A(IE+I))-B(IC+I))                         &
     &    +SSIN45*((A(IB+I)-A(ID+I))-(B(IB+I)+B(ID+I)))
        T5=2.0_4*((A(IA+I)-A(IE+I))-B(IC+I))                         &
     &    -SSIN45*((A(IB+I)-A(ID+I))-(B(IB+I)+B(ID+I)))
        T3=2.0_4*((A(IA+I)-A(IE+I))+B(IC+I))                         &
     &    -SSIN45*((A(IB+I)-A(ID+I))+(B(IB+I)+B(ID+I)))
        T7=2.0_4*((A(IA+I)-A(IE+I))+B(IC+I))                         &
     &    +SSIN45*((A(IB+I)-A(ID+I))+(B(IB+I)+B(ID+I)))
        T4=2.0_4*(((A(IA+I)+A(IE+I))+A(IC+I))-(A(IB+I)+A(ID+I)))
        A(IA+I)=2.0_4*(((A(IA+I)+A(IE+I))+A(IC+I))+(A(IB+I)+A(ID+I)))
        A(IB+I)=T1
        B(IB+I)=T2
        A(IC+I)=T3
        B(IC+I)=T4
...
!NEC$ ivdep
        DO 830 IJK=1,ILOT
        C(JA+J)=2.0_4*(((A(IA+I)+A(IE+I))+A(IC+I))+(A(IB+I)+A(ID+I)))
        C(JE+J)=2.0_4*(((A(IA+I)+A(IE+I))+A(IC+I))-(A(IB+I)+A(ID+I)))
        C(JC+J)=2.0_4*(((A(IA+I)+A(IE+I))-A(IC+I))-(B(IB+I)-B(ID+I)))
        C(JG+J)=2.0_4*(((A(IA+I)+A(IE+I))-A(IC+I))+(B(IB+I)-B(ID+I)))
        C(JB+J)=2.0_4*((A(IA+I)-A(IE+I))-B(IC+I))                    &
     &    +SSIN45*((A(IB+I)-A(ID+I))-(B(IB+I)+B(ID+I)))
        C(JF+J)=2.0_4*((A(IA+I)-A(IE+I))-B(IC+I))                    &
     &    -SSIN45*((A(IB+I)-A(ID+I))-(B(IB+I)+B(ID+I)))
        C(JD+J)=2.0_4*((A(IA+I)-A(IE+I))+B(IC+I))                    &
     &    -SSIN45*((A(IB+I)-A(ID+I))+(B(IB+I)+B(ID+I)))
        C(JH+J)=2.0_4*((A(IA+I)-A(IE+I))+B(IC+I))                    &
     &    +SSIN45*((A(IB+I)-A(ID+I))+(B(IB+I)+B(ID+I)))
        I=I+INC3
        J=J+INC4
  830   CONTINUE
...
!
      ...
      REAL(KIND=4) :: D(*)
      REAL(KIND=4) :: TRIGS(N)
      REAL(KIND=4) :: A0,A1,A2,A3,A4,A5,A6,A10,A11,A20,A21
      REAL(KIND=4) :: B0,B1,B2,B3,B4,B5,B6,B10,B11,B20,B21
      REAL(KIND=4) :: C1,C2,C3,C4,C5
      REAL(KIND=4) :: S1,S2,S3,S4,S5
      REAL(KIND=4) :: T1,T2,T3,T4,T5,T6,T7
      REAL(KIND=4) :: Z
      REAL(KIND=4) :: QRT5,SIN36,SIN45,SIN60,SIN72
      REAL(KIND=4) :: ZQRT5,ZSIN36,ZSIN45,ZSIN60,ZSIN72
      INTEGER(KIND=4) :: IERR
      INTEGER(KIND=4) :: INC1
      INTEGER(KIND=4) :: INC2
      INTEGER(KIND=4) :: INC3
      ...
      INTEGER(KIND=4) :: IFAC
      INTEGER(KIND=4) :: LA
      INTEGER(KIND=4) :: IINK,IJK,ILOT
      INTEGER(KIND=4) :: I,IA,IB,IBAD,IBASE,IC,ID,IE,IF,IG,IH
      INTEGER(KIND=4) :: IJUMP,ILA,INC11
      INTEGER(KIND=4) :: J,JA,JB,JC,JD,JE,JBASE,JF,JINK
      INTEGER(KIND=4) :: K,KB,KC,KD,KE,KF,KSTOP
      INTEGER(KIND=4) :: L,M
      LOGICAL :: LIPL
!
...
!NEC$ ivdep
      DO 310 IJK=1,ILOT
      C(JA+J)=A(IA+I)+(A(IB+I)+A(IC+I))
      C(JB+J)=A(IA+I)-0.5_4*(A(IB+I)+A(IC+I))
...
!NEC$ ivdep
      DO 370 IJK=1,ILOT
      C(JA+J)=A(IA+I)+0.5_4*(A(IB+I)-A(IC+I))
...
!NEC$ ivdep
        DO 392 IJK=1,ILOT
        T1=Z*(A(IA+I)-0.5_4*(A(IB+I)+A(IC+I)))
...
!NEC$ ivdep
        ...
        C(JB+J)=Z*(A(IA+I)-0.5_4*(A(IB+I)+A(IC+I)))
...
!NEC$ ivdep
      ...
      A11=(A2+A5)+(A1+A4)
      A20=(A(IA+I)+A3)-0.5_4*A11
      A21=SIN60*((A2+A5)-(A1+A4))
      B11=(B2+B5)+(B1+B4)
      B20=(B(IA+I)+B3)-0.5_4*B11
...
!NEC$ ivdep
      DO 670 IJK=1,ILOT
      C(JA+J)=(A(IA+I)+0.5_4*(A(IC+I)-A(IE+I)))+                     &
     &         SIN60*(A(IB+I)-A(IF+I))
      D(JA+J)=-(A(ID+I)+0.5_4*(A(IB+I)+A(IF+I)))-                    &
     &         SIN60*(A(IC+I)+A(IE+I))
      C(JB+J)=A(IA+I)-(A(IC+I)-A(IE+I))
      D(JB+J)=A(ID+I)-(A(IB+I)+A(IF+I))
      C(JC+J)=(A(IA+I)+0.5_4*(A(IC+I)-A(IE+I)))-                     &
     &         SIN60*(A(IB+I)-A(IF+I))
      D(JC+J)=-(A(ID+I)+0.5_4*(A(IB+I)+                              &
...
!NEC$ ivdep
        ...
        C(JC+J)=Z*((A(IA+I)+A(ID+I))-0.5_4*A11)
        D(JC+J)=ZSIN60*((A(IC+I)+A(IF+I))-(A(IB+I)+A(IE+I)))
        A11=(A(IC+I)-A(IF+I))+(A(IE+I)-A(IB+I))
        C(JB+J)=Z*((A(IA+I)-A(ID+I))-0.5_4*A11)
